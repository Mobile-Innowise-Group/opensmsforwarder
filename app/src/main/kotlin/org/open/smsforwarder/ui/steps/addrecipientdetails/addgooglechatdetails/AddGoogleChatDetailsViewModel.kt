...
import org.open.smsforwarder.domain.usecase.ValidateGoogleChatWebHookUseCase
...
import org.open.smsforwarder.ui.mapper.toGoogleChatDetailsUi

@HiltViewModel(assistedFactory = AddGoogleChatDetailsViewModel.Factory::class)
class AddGoogleChatDetailsViewModel @AssistedInject constructor(
    @Assisted private val id: Long,
    private val forwardingRepository: ForwardingRepository,
    private val validateGoogleChatWebHookUseCase: ValidateGoogleChatWebHookUseCase,
    private val router: Router,
) : ViewModel(), DefaultLifecycleObserver {

    private var _viewState = MutableStateFlow(AddGoogleChatDetailsState())
    ...
    private fun loadData() {
        launchAndCancelPrevious {
            forwardingRepository
                ...
                .collect { forwarding ->
                    _viewState.update {
                        forwarding.toGoogleChatDetailsUi()
                    }
                }
        }
    }

    fun onGoogleChatWebHookChanged(googleChatWebHook: String) {
        val googleChatWebHookValidationResult = validateGoogleChatWebHookUseCase.execute(googleChatWebHook)
        _viewState.update {
            it.copy(
                googleChatWebHook = googleChatWebHook,
                inputErrorGoogleChat = googleChatWebHookValidationResult.errorType?.getStringProvider()
            )
        }
    }
    ...
    @AssistedFactory
    interface Factory {
        fun create(id: Long): AddGoogleChatDetailsViewModel
    }
}
